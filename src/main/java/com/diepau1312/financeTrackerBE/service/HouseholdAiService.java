...
public class HouseholdAiService {
  ...
  public RestockPredictionDTO predictRestock(UUID itemId) {
    ...
    for (int i = 0; i < history.size() - 1; i++) {
      long days = ChronoUnit.DAYS.between(history.get(i + 1).getPurchaseDate(), history.get(i).getPurchaseDate());
      if (days > 0) {
        totalDays += days;
        intervals++;
      }
    ...
    String explanation = String.format(
        "Dua vao %d lan mua gan day, ban thuong mua lai %s sau khoang %d ngay. Uoc tinh con khoang %d ngay nua can mua them.",
